		...
		function time(obj) {
			...
			timer = setInterval(function() {
				if (time == 0) {
					...
					obj.html('120秒后重新发送');
					$('.valBtn').html("重新发送");
					$('sendVal').removeClass('sendValTime');
					$('.valBtn').show();
					obj.hide();
				} else {
					time--;
				...
				if (data == 4) {
					...
					if(getadd("mmtoken")){
						...
						$.ajax({
							   ...
							   success: function (data) {
							     window.location.href = environment.basePath + "/recharge/openAccount.session";
							   },error: function(data){
								  window.location.href = environment.basePath + "/recharge/openAccount.session";
							   }
							});
